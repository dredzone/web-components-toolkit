	...
	return class Properties extends baseClass {
		...
		static get observedAttributes() {
			return Object.keys(this.classProperties)
				.map((property) => this.propertyNameToAttribute(property)) || [];
		}
		...
		_createPropertyAccessor(property, readOnly) {
			if (!dataHasAccessor[property]) {
				...
				defineProperty(this, property, {
					...
					set: readOnly ? () => {} : function (newValue) {
		...
		_attributeToProperty(attribute, value) {
			if (!storage(this).serializing) {
				const property = this.constructor.attributeToPropertyName(attribute);
				this[property] = this._deserializeValue(property, value);
			}
		}

		_isValidPropertyValue(property, value) {
			const propertyType = this.constructor.classProperties[property].type;
			let isValid = false;
		...
		_serializeValue(property, value) {
			const propertyConfig = this.constructor.classProperties[property];
		...
		_flushProperties() {
			const props = storage(this).data;
			const changedProps = storage(this).dataPending;
			const old = storage(this).dataOld;
