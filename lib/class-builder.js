...
export const createMixin = (mixin) => dedupeMixin(
	cacheMixin(
		declareMixin(mixin)
	)
);

export default (Ctor = class {}) => freeze({
	with(...mixins) {
		return mixins
...
function isApplicationOf(proto, mixin) {
	return (
		hasOwnProperty.call(proto, appliedMixinKey) && proto[appliedMixinKey] === unwrap(mixin)
	);
}
